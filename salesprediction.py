...
df1.loc[df1['outlet_number'] <= 19, 'outlet_number'] = 1
df2.loc[df2['outlet_number'] <= 19, 'outlet_number'] = 1
...
# Suppose df is your DataFrame and 'Item_Type' is the column to encode
le = LabelEncoder()
...
bins = [0, 70, 140, 200, df1['Item_MRP'].max()]
labels = ['Low', 'Medium', 'High', 'Very High']

# Apply binning
df1['MRP_Binned'] = pd.cut(df1['Item_MRP'], bins=bins, labels=labels)
encoder = OrdinalEncoder(categories=[['Low', 'Medium', 'High', 'Very High']])  
df1["MRP_bin_ecoded"] = encoder.fit_transform(df1[["MRP_Binned"]])
df1=df1.drop(columns = "MRP_Binned")

df2['MRP_Binned'] = pd.cut(df2['Item_MRP'], bins=bins, labels=labels)
encoder = OrdinalEncoder(categories=[['Low', 'Medium', 'High', 'Very High']])  
df2["MRP_bin_ecoded"] = encoder.fit_transform(df2[["MRP_Binned"]])
df2=df2.drop(columns = "MRP_Binned")
...
# Base models wrapped in pipelines with scaler
base_models = [
...
# Meta-model also in a pipeline (optional)
